...
- **DRAFT** (đang chỉnh, gần sẵn sàng):
  ...
  - Master Index v1.5.23 (pending re-lock)
...
| `00_DSTS_MASTER_INDEX_2026.md` | Master index 3 Layer + Phase 0B + NDNUM | 📝 DRAFT v1.5.14 — pending re-lock |
...
| `../assets/app-v5.js` | Current public app shell; retired `assets/app.js` is blocked from deploy | 🟢 Current |
| `../_headers` | Origin header/cache policy validated by content QA | 🟢 Current |
| `../_redirects` | Clean redirects and Movement placeholder routes validated by content QA | 🟢 Current |
| `../_routes.json` | Pages Functions include/exclude routing validated by content QA | 🟢 Current |
...
- Dùng `scripts/content-depth-qa.mjs` để khóa độ sâu fallback content: 24 bài post và 2 page fallback phải có đủ số từ, heading, paragraph, excerpt, tag, reading time và không có placeholder.
- Dùng `scripts/static-page-depth-qa.mjs` để khóa độ sâu các trang HTML public tĩnh quan trọng: tối thiểu 180 từ, 2 heading section và không còn marker loading/placeholder giả.
- Dùng `scripts/link-qa.mjs` để khóa link nội bộ trên preview/production: quét 32 route public, mọi `href/src/action` nội bộ phải trả 2xx/3xx.
